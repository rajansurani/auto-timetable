...
import { deleteFacultyByCode, fetchFaculties, updateFacultyByCode } from "../../../database/dataAccess";
...
function Faculty() {
	...
	const updateFacultiesList = () => {
		console.log("Faculty Fetch Start");
	...
	const handleFacultyDelete = () => {
		deleteFacultyByCode(selectedFaculty.facultyCode).then(() => {
			updateFacultiesList();
	...
	return (
		<div>
												...
												<tr key={idx}>
													...
													<td>
														{item.assignedSubjects.map((subject, index) => (
															<div>
																<span className="badge bg-primary p-2 m-1">
																	{subject}
																</span>
															</div>
														))}
			...
			<Modal show={showDeleteDialog} onHide={handleCloseDeleteDialog}>
				...
				<Modal.Body>
					You are about to delete {selectedFaculty.facultyCode} -{" "}
				...
				<Modal.Footer>
					...
					<Button variant="primary" onClick={handleFacultyDelete}>
						Save Changes
					</Button>
				</Modal.Footer>
			</Modal>
