...
import { addCourseAllocation, deleteCourseAllocationById, deleteCourseByCode, fetchCourseAllocation, fetchCourses, fetchTimetableByVersion, updateCourseByCode } from "../../../../database/dataAccess";
...
import EditCourseAllocation from "./EditCourseAllocation";

function CourseAllocation(props){

    const [courseAllocationList, setCourseAllocationList] = useState([]);
	...
	const updateCourseAllocationList = () => {
		setLoadingOverlay(true);
		fetchCourseAllocation(timetableVersion).then((courseAllocations) => {
			setCourseAllocationList(courseAllocations);
			setLoadingOverlay(false);
		});
	};

	const handleCourseAllocationDelete = () => {
		deleteCourseAllocationById(selectedCourseAllocation.allocationId).then(() => {
			updateCourseAllocationList();
    ...
    return(
        <div>
						...
						<Card>
							<Card.Header className="d-flex justify-content-between">
								<div className="header-title">
									<h4 className="card-title">Course Alloction for {timetableVersion}</h4>
								</div>
									...
									<table
										...
										<thead>
											<tr className="ligth">
												...
												<th>Batch A - Faculty</th>
												<th>Batch B - Faculty</th>
												<th>Batch C - Faculty</th>
												<th min-width="100px">Action</th>
											</tr>
										</thead>
										<tbody>
											{courseAllocationList.map((item, idx) => (
												<tr key={idx}>
													<td>
														{item.batchName}
													</td>
                                                    ...
                                                    <th>{item.theoryFaculty}</th>
                                                    <td>
														{item.numberOfBatch}
													</td>
													<td>{item.batchAFaculty}</td>
													<td>{item.batchBFaculty}</td>
													<td>{item.batchCFaculty}</td>
													<td>
														<div className="flex align-items-center list-user-action">
															<Button
																...
																onClick={() => {
																	setSelectedCourseAllocation(item);
			...
			<Modal show={showDeleteDialog} onHide={handleCloseDeleteDialog}>
				...
				<Modal.Body>
					You are about to delete {selectedCourseAllocation.courseCode} for {selectedCourseAllocation.batchName}.
				</Modal.Body>
				<Modal.Footer>
					<Button variant="secondary" onClick={handleCloseDeleteDialog}>
						Close
					</Button>
					<Button variant="primary" onClick={handleCourseAllocationDelete}>
						Save Changes
					</Button>
				</Modal.Footer>
			</Modal>
			...
			{showEditDialog ? (
				<EditCourseAllocation
					courseAllocation={selectedCourseAllocation}
                    timetable = {timetable}
					addCourseAllocation={dialogTypeAdd}
